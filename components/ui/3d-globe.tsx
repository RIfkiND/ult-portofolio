    ...
    <group ref={groupRef} visible={isVisible}>
      ...
      <group ref={imageGroupRef} position={topPosition}>
        ...
        >
          <div
            className={cn(
              "cursor-pointer overflow-hidden rounded-full bg-white shadow-lg transition-all duration-200 flex items-center justify-center",
              hovered && "scale-[1.08] shadow-xl ring-1 ring-white/50",
            )}
            style={{
...
function Scene({ markers, config, onMarkerClick, onMarkerHover }: SceneProps) {
  ...
  // Set initial camera position based on initialRotation config
  React.useEffect(() => {
    const dist = config.radius * 3.5;
    const phi = (Math.PI / 2) - config.initialRotation.x; // vertical angle
    const theta = config.initialRotation.y; // horizontal angle (longitude-based)
    ...
    camera.lookAt(0, 0, 0);
  }, [camera, config.radius, config.initialRotation.x, config.initialRotation.y]);
